...
**Статус:** РЕАЛИЗОВАНО, обе фазы (сверено с кодом 2026-08-24). Вариант B (обобщённый раннер, по
сайдкару/папке на платформу), зеркало `tgCollect`.

- **Фаза A** (свой процесс, своя кнопка, свои часы, адаптерное исполнение) — `scheduler.ts`,
  `usePosting_store.ts`, кнопка «START POSTING» в `AppMain`, интервал обхода из настроек
  (`posting.scanWaitMin`).
- **Фаза B** («вариант Y»: Finder-источник + Poster-плагин + исполнение общим `processItem`) —
  тоже закрыта: `src/PROCESSING/autoPost/index.ts` строит work-item и зовёт `processItem`,
  плагин `finder` — нода-источник (`deleteAfter` на ней), `autoPostVK` / `autoPostYT` — настоящие
  Poster-ноды с выходом в граф, сайдкар единый `options/postSources.json`, платформа выводится
  из графа (`posters.ts`).

Целевая архитектура, которая этот документ заменила, —
[`UNIFIED_SOURCES_ENGINE.md`](./UNIFIED_SOURCES_ENGINE.md); здесь остаётся история решений.

**Хвосты фазы A, которые фаза B оставила мёртвыми** (0 импортов, проверено 2026-08-24):
`src/PROCESSING/autoPost/adapters/vk.ts` (148 строк) и `src/PROCESSING/autoPost/logWin.ts`
(72 строки). Исполнение адаптером заменено на `processItem`, логи идут от движка (в `index.ts`
своя локальная функция `logWin`). Удаление — отдельным решением, план их больше не требует.

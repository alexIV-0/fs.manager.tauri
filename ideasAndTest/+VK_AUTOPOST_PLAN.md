...
## Статус
🟢 РЕАЛИЗОВАНО (сверено с кодом 2026-08-24) — в объёме режима **Video (Post)**.
Плагин `plugins-dev/autoPostVK/` — настоящая Poster-нода (вход `inputFile`, выход в граф),
публикация в `_publisher.ts`, авторизация — `src-tauri/src/commands/vk_auth_commands.rs`,
исполнение — общий раннер `src/PROCESSING/autoPost/` через `processItem`
(см. [`+AUTOPOST_DECOUPLED_PLAN.md`](./+AUTOPOST_DECOUPLED_PLAN.md)).

⏸ **Клипы / Both в UI не включены — отложено осознанно (2026-08-24).** Код публикации клипа в `_publisher.ts` есть целиком
(`shortVideo.create` → upload → пауза на обработку → `edit` → `publish`), но свойства выбора
режима в `ui.json` нет, и нода всегда пишет `mode: 'video'` — см. комментарий в `_publisher.ts`.
Чтобы расконсервировать: свойство режима в `ui.json` + проводка до `publishVideo`.

История: 🎉 ПРОРЫВ (2026-06-15 вечер) — **все 3 режима подтверждены вживую** через токен
**vk.com (6287487)**, включая Клипы и Both.

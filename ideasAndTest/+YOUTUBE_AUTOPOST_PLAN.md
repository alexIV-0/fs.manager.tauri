...
**Статус:** 🟢 РЕАЛИЗОВАНО (сверено с кодом 2026-08-24). Пункты 1–4 раздела 8 закрыты:
`youtube_auth_commands.rs` (`youtube_auth_start`, `youtube_refresh_token`,
`youtube_get_access_token`), `youtube_upload_commands.rs` (`youtube_upload_video`,
`youtube_set_thumbnail`), плагин `plugins-dev/autoPostYT/` (Poster-нода, вход `inputFile`,
выход в граф, запись в `_post`-лог), платформа `youtube` в `posters.ts`. Осталось внешнее и
не наше: пункт 5 — audit проекта у пользователя, до него видео уходят `private` силами Google.
...
- **Квота — его** (10 000 юнитов/день на ЕГО проект). Нет общего потолка на всех — в отличие
  от Model A. ⚠️ **Исправлено 2026-08-24:** здесь стояло «`videos.insert` ≈ 100 юнитов →
  ~100 загрузок/день» — это ошибка. `videos.insert` стоит **1600 юнитов**, то есть дефолтная
  квота = **≈6 загрузок в сутки на проект**, и это самый жёсткий предел всей затеи.
  Больше — только запросом на увеличение квоты у Google (отдельная процедура с обоснованием).
...
2. ✅ **Rust upload-спайк СДЕЛАН:** `youtube_upload_video` + `youtube_set_thumbnail`
   (`src-tauri/src/commands/youtube_upload_commands.rs`), resumable upload и метаданные.
3. ✅ **Плагин `autoPostYT` СДЕЛАН:** auth-UI + Poster-нода (`plugins-dev/autoPostYT/`,
   136 строк), account-стор, запись в `_post`-лог через `postLog.ts`.
   Токен refresh-aware: `youtube_get_access_token` обновляет `access_token` сам.
4. ✅ **Раннер СДЕЛАН:** единый сайдкар `options/postSources.json`, платформа per finder —
   `POSTER_PLATFORM` в `src/PROCESSING/autoPost/posters.ts`.
...
| Квота | Пользователь | Его 10 000 юнитов/день; `videos.insert` = 1600 юнитов → **≈6 загрузок/день** |

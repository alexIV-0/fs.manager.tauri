...
Текущее устройство — `ARCHITECTURE.md`. Настройка процессов под конкретную задачу — `flows/` (не про устройство программы, а про то, как собрать граф и что он обязан выдать; указатель — `flows/README.md`). Планы будущего — `ideasAndTest/`; там описаны и абстракции, которых в коде ещё нет (список — в разделе «Спроектировано, но не существует» карты). Файл с `+` в начале имени = план реализован. Что в папке есть и в каком состоянии — `ideasAndTest/README.md` (снимок ревизии; истина про конкретный план — в шапке самого плана).

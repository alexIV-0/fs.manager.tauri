...
export const ByMonthTemplate: ITemplate = {
  id: 'by-month',
  label: 'По месяцам',
  description: 'Группирует статистику по месяцам и проектам',
  isBuiltIn: true,
  ...
  getConfigs(storage: StorageSettings): unknown[] {
    return storage.localArchives?.filter((a) => a.templateId === 'by-month') ?? [];
  },
  ...
  async write(data: unknown, _context: SaveContext, config: unknown): Promise<void> {
    ...
    const { monthKey } = getDateParts(record.startedAt || record.registeredAt);

    existing[monthKey] = existing[monthKey]
      ? mergePeriodStats(existing[monthKey]!, record)
    ...
    try {
      ...
      throw new Error(`Failed to write by month stats: ${(e as Error).message}`);
    }
  },
};

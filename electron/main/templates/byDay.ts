...
export const ByDayTemplate: ITemplate = {
  id: 'by-day',
  label: 'По дням',
  description: 'Группирует статистику по дням (YYYY-MM-DD) и проектам',
  isBuiltIn: true,
  ...
  getConfigs(storage: StorageSettings): unknown[] {
    return storage.localArchives?.filter((a) => a.templateId === 'by-day') ?? [];
  },
  ...
  async write(data: unknown, _context: SaveContext, config: unknown): Promise<void> {
    ...
    const { dayKey } = getDateParts(record.startedAt || record.registeredAt);

    existing[dayKey] = existing[dayKey]
      ? mergePeriodStats(existing[dayKey]!, record)
    ...
    try {
      ...
      throw new Error(`Failed to write by day stats: ${(e as Error).message}`);
    }
  },
};

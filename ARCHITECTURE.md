...
- `ResourcePool.ts` — семафоры по **классу ресурса** (`local` / `online` / `ffmpeg` / `helpers`), не по цвету ноды; лимиты из `src/types/appSettings.ts`, а набор семафоров — **на прогон**, не на процесс (см. «Полосы прогонов» ниже)
...
**Полосы прогонов — то, что легко сломать заново.** Раннеров ДВА: обработка и постинг. Кнопки разные, часы разные, работать могут одновременно. Всё, что у них общее по природе, обязано быть разделено по **полосе** (`src/PROCESSING/runLanes.ts`: `processing` / `posting`):

- набор семафоров (`createRunPools(lane, …)` / `disposeRunPools(lane)`);

...
/**
 * Схема санитайза для ПОДСКАЗОК свойств нод — тот же формат, но без блочных
 * конструкций.
 *
 * Подсказка живёт в маленьком поповере и только показывает готовый текст:
 * таблице, картинке, спойлеру и блок-схеме там делать нечего, а цвет как раз
 * нужен (им выделяют «важно» и «осторожно»). Отдельная схема, а не
 * `descriptionSanitizeSchema`, потому что автор плагина может принести в
 * `tooltip` что угодно: строка едет из `ui.json` и правится руками.
 *
 * Санитайз выбрасывает ТЕГ, но оставляет его содержимое — поэтому таблица из
 * старой подсказки не исчезнет, а покажется строками текста.
 */
export const tooltipSanitizeSchema: SanitizeSchema = {
	...descriptionSanitizeSchema,
	tagNames: (descriptionSanitizeSchema.tagNames ?? []).filter(
		(t) => !['table', 'thead', 'tbody', 'tfoot', 'tr', 'th', 'td', 'img', 'details', 'summary', 'input'].includes(t),
	),
};

...
// ========================
// РЕАКТИВНЫЕ ХУКИ НАЛИЧИЯ ПЛАГИНОВ
// ========================

/**
 * Реактивный флаг: установлен ли (включён И присутствует на диске) хотя бы один
 * из плагинов с указанными id. Для условного показа/скрытия UI по наличию плагина
 * (кнопки, секции настроек, подсказки «поставь плагин» через `!useHasAnyPlugin(...)`).
 * Пустой список → false. Возвращает boolean, поэтому Object.is-сравнение zustand
 * не даёт лишних ре-рендеров.
 */
export const useHasAnyPlugin = (ids: string[]): boolean =>
	plugin_Store((s) => ids.some((id) => s.plugins.some((p) => p.id === id && p.enabled && p.exists)));
